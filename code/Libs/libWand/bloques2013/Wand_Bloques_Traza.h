...
//calculo real, no hay nada que modificar....
        ...
        vector<unsigned int> scoreCurrentDocTrazaUb(const unsigned int docId, const int *streamIdx, const int startIdx, const int endIdx, unsigned int &result){
                count++;
                vector<unsigned int> ubs;
                unsigned int score = 0;
                for (int i = startIdx; i <= endIdx; ++i) {
                        score += (*streams)[streamIdx[i]]->payload()
                                        * (*query_terms)[streamIdx[i]]->weight;
                        ubs.push_back((*query_terms)[streamIdx[i]]->id_term);                
                        ubs.push_back((*streams)[streamIdx[i]]->payload());
...
//int currentPayload;	
	...
	vector<PostingStream*> *streams;
        ...
        map<unsigned int, vector<unsigned int> > UbsDocs;
				...
				scorer->scoreCurrentDoc(currentDocId, streamIndices, 0, pivotIdx, currentPayload);
   				 ...
   				 scorer->scoreCurrentDocIntervalos(currentDocId, streamIndices, 0, pivotIdx, currentPayload);
                                ...
                                if(opcion==3)
                                {
                                    ...
                                    //  
                                            UbsDocs[currentDocId] = scorer->scoreCurrentDocTrazaUb(currentDocId, streamIndices, 0, pivotIdx, currentPayload);
...
//arreglar esta funcion es vital para el conteo
	...
	bool findPivotFeatureIdx(const unsigned int threshold, int &pivotIdx,int &UB){
		...
		scorer->resetForPivotFeatureSelection();//score = 0;
                bool response=false;
