...
//	unsigned int 
	...
	PostingStream(int _n, unsigned int *_id_doc_l, unsigned int *_payload_l){
        ...
        PostingStream(list< pair<unsigned int, unsigned int> > *lista,vector<double> UbBloques,vector<unsigned int> idDocFronteras,vector<unsigned int> idDocFronterasFin,double _ub_inicial){
...
public:
	...
	double ub_ini;
	double ub;
    double *ub_bloques_intervalo; //ub locales
...
public:
	...
	vector<QueryFeatureInfoT*> *query_terms;
        ...
        vector<vector<double> > query_terms_bloques_pesos;
...
//calculo real, no hay nada que modificar....
        ...
        void scoreCurrentDocIntervalosBM25(const unsigned int docId, const int *streamIdx, const int startIdx, const int endIdx, double &result){
                count++;
                double score = 0.0;
                int sc1;
                int dimTerm;
...
//                        streams = NULL;
	...
	void init(){
		...
		evaluacionesCompletas=0;
                ...
                streamUB = new double[numStreams];
	...
	//	cout<<"Wand::next - inicio (threshold: "<<threshold<<")\n";
		...
		while (true) {
			...
			if (streamDocIds[streamIndices[0]] == streamDocIds[streamIndices[pivotIdx]])
			{
				...
				/*while (pivotIdx < numStreams-1 && streamDocIds[streamIndices[pivotIdx+1]] == pivotDocId) {
   				...
   				//BM25
                scorer->scoreCurrentDocIntervalosBM25(currentDocId, streamIndices, 0, pivotIdx, currentPayload);
				}                
        ...
        int next(double threshold/*,int opcion*/){
...
*/
        ...
        bool CheckBlockMax(const double threshold,int pivot,unsigned int d)
        {
            double maxposs = 0.0;
        ...
        unsigned int GetNewCandidate(int frontier,int &posMin,int &posMax,unsigned int docin)
        {
            ...
            double Dmax = scorer->ub_query_terms(streamIndices[0],bloqueNow[streamIndices[0]]);
            posMin = 0;
            posMax = 0;
