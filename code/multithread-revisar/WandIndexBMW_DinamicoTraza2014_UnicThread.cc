     ...
     // else
      ...
      typedef pair <int, sizeListas> tid_sizeListas;
       ...
       // Tiempo ini preprocesamiento querie
        clock_gettime(CLOCK_REALTIME, &tiempo1_PreQ);
...
clock_gettime(CLOCK_REALTIME, &tiempo2_PreQ);


//int preCalculados=0;
         //tiempo ini WAND
            clock_gettime(CLOCK_REALTIME, &tiempo1_Wand);
...
//clock_gettime(CLOCK_MONOTONIC, &tiempo2_IteWand);
 ...
 //Tiempo fin WAND
  clock_gettime(CLOCK_REALTIME, &tiempo2_Wand);
 //Tiempo inicial Merge Sort
  clock_gettime(CLOCK_REALTIME, &tiempo1_MergeSort);
    int contador=1;
...
clock_gettime(CLOCK_REALTIME, &tiempo2_MergeSort);
...
unsigned long int tiempoWand_procQ;
/*
   ...
   if(tidLocal==0)
{
 ...
 sprintf (nombreArchivoConsulta, "/run/media/oscar/Informacion/universidad/Doctorado/2013/2014/funcionesDensidadGlobal/Tiempos_T%d_%d_K%dBMW_UNICAS_Datos_y_WANDMerge10",nT,T,ki);
printf ("/run/media/oscar/Informacion/universidad/Doctorado/2013/2014/funcionesDensidadGlobal/log_HeapLocales_Tiempos_T%d_Consultas%d_K%dBMW_UNICAS_Dincamic.dat",nT,T,ki);
...
for( unsigned int i=0;i<T;i++)
{
 ...
 //pthread_barrier_wait(&barrier);
  ...
  //  pthread_mutex_lock( &lock );
   ...
   // if(contadorLocal==1)
   // {
        clock_gettime(CLOCK_REALTIME, &tiempo1_ProcQ); //fin tiempo Iterador 
...
// pthread_barrier_wait(&barrier); //este barrier no afecta toma de tiempo en sort heap, pero en implementacion en motor de busqueda destruir el heap global una vez enviado querie al front
 clock_gettime(CLOCK_REALTIME, &tiempo2_ProcQ);
          if(tidLocal==0)
          {  
              //Procesamiento Q
              ...
              thersholdLogConsulta<<" "<<tiempoWand_procQ;      		   //Procesaiento WAND
			       
			       
			  diff_time(&tiempo2_MergeSort,&tiempo1_MergeSort,&latencia); 
              tiempoWand_procQ=latencia.tv_sec*1000000000+latencia.tv_nsec;
              thersholdLogConsulta<<" "<<tiempoWand_procQ;      		   //Procesaiento Merge
			         
			       
			       
              diff_time(&tiempo2_PreQ,&tiempo1_PreQ,&latencia);            //Pre-procesamiento WAND
              tiempoWand_procQ=latencia.tv_sec*1000000000+latencia.tv_nsec;
              thersholdLogConsulta<<" "<<tiempoWand_procQ;

			  diff_time(&tiempo2_ProcQ,&tiempo1_ProcQ,&latencia); 
              tiempoWand_procQ=latencia.tv_sec*1000000000+latencia.tv_nsec;
              thersholdLogConsulta<<" "<<tiempoWand_procQ<<endl;        //Procesamiento Q
...
{   
 ...
 // sprintf (nombreArchivoConsulta, "./logs/log_Umbral_HeapLocales_Costos_T%d_Consultas%d_K%ddFourier.dat",nT,T,K);
 // sprintf (nombreArchivoGlobales, "./logs/log_Global_HeapLocales_Costos_T%d_Consultas%d_K%d.dat",nT,T,K);
 sprintf (nombreArchivoConsulta, "/run/media/oscar/Informacion/universidad/Doctorado/2013/2014/funcionesDensidadGlobal/log_HeapLocales_Tiempos_T%d_Consultas%d_K%dBMW_UNICAS",nT,T,ki);
...
leerConsultasChar("/run/media/oscar/Informacion/universidad/Doctorado/2013/2014/funcionesDensidadGlobal/myFile2.txt",&querys,numeroConsultasProceso);
...
struct timespec latencia;
clock_gettime(CLOCK_REALTIME, &tiempo1_Lista);
...
clock_gettime(CLOCK_REALTIME, &tiempo2_Lista);
