...
void cargaListas(int tid, int nPaticionIn)
{
...
//i7 - 25Gb
//sprintf (nombreArchivoLista, "/run/media/oscar/Datos/datos_gov2/log3mb/lista25Gb/gov2_8thread/listaGov26G_C8_Thread_0.dat");
printf ("/run/media/oscar/Datos/datos_gov2/log3mb/lista25Gb/gov2_%dthread/listaGov26G_C%d_Thread_%d.dat",nPaticionIn,nPaticionIn,tid);
sprintf (nombreArchivoLista, "/run/media/oscar/Datos/datos_gov2/log3mb/lista25Gb/gov2_%dthread/listaGov26G_C%d_Thread_%d.dat",nPaticionIn,nPaticionIn,tid);

////sprintf (nombreArchivoLista, "/run/media/oscar/Datos/datos_gov2/log3mb/lista25Gb/gov2_1thread/listaGov26G_C1_Thread_%d.dat",t);
leerIndiceCharGrupos(nombreArchivoLista,tid);
//}
...
nPaticionIn = numThreads; //numero de particiones de listas a utilizar
...
sprintf (nombreArchivoTiempo, "/run/media/oscar/Informacion/universidad/Doctorado/2013/2014/funcionesDensidadGlobal/trazaThreadOptimoRendimiento/junio2014/Tiempos_T%d_%d_KGlobales",numThreads,T);
...
{   
 ...
 clock_gettime(CLOCK_REALTIME, &tiempo1_Padre); 
 cout<<" :" <<ki<<endl;
...
//CPU_SET( t, &set );
   ...
   // printf("Heap Locales\n");
    /*int s= pthread_setschedparam(mythread[t], SCHED_FIFO, &fifo_param);
     if (s != 0)
        handle_error_en(s, "pthread sin asigancion fifo");
...
//pthread_barrier_wait(&barrier);
 ...
 clock_gettime(CLOCK_REALTIME, &tiempo2_Padre); 
 diff_time(&tiempo2_Padre,&tiempo1_Padre,&latencia); 
 tiempoPadre=latencia.tv_sec*1000000000+latencia.tv_nsec;


cout<<"Tiemo Global: "<<tiempoPadre<<endl; 
tiempoGlobal<<tiempoPadre<<" "; 
