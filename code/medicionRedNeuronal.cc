					...
					//   logWand[tmpFile]<<" "<<efectiveWAND;
					  ...
					  pthread_rwlock_unlock(&rw_rdlock);
					  		...
					  		//	pthread_rwlock_wrlock(&rw_rdlock);
					            ...
					            {
									//pthread_rwlock_wrlock(&rw_rdlock);
									liberarBanderaThreads(tmp->vBanderas);
...
map<int,unsigned long int> predictor;
void prediccion()
{
 std::ifstream ifs ("./prediccion.txt", std::ifstream::in);

    string linea,dato;
    int q;
    int contador = 0;
    unsigned long int tiempoQ;
...
int asignaThreadsSheduler(int q)
{
	...
	if(predictor[q] < 300000000)
	return 1;
	if(predictor[q] >= 300000000 && predictor[q] <= 600000000 )
	return 2;
	if(predictor[q] > 600000000)
	return 4;
  ...
  //simulacion de tiempo deprocesamiento
	void funcionDormir(float _time)
	{    
...
clock_gettime(CLOCK_MONOTONIC, &tiempo1_GlobalQLote);
		...
		if(metodo==42)
		{
			for(int j=0;j<14;j++)
			{
				...
				//sumaDescriptores += 0.654;
				
		//		cout<<(double)varianzaW(w_t)<<" varianzaW(w_t)"<<endl;
			varianzaW(w_t);
			for(int i=0;i<termsIN;i++)
				sumaDescriptores += 0.654;	
				//el maximo se puede optimizar...
...
cout<<(double)tiempo<<" ns. build vector"<<endl;
cout<<(double)tiempo/1000000<<" ms. build vector"<<endl;
cout<<(double)tiempo/1000000000<<" sec. build vector"<<endl;	
	...
	//suma = 0.0;
		for(int i=0;i<entradas;i++)
		{
			...
			//funcionDormir(0.5);
			numberX = ( rand() )/1000000000.0;
		//	numberW = ( rand() )/1000000000.0;
			//cout<< numberX<<" numberX"<<endl;
			//cout<< numberW<<" numberW"<<endl;
			pesos[i][k] = numberX;
			//suma = suma + numberX*numberW;
		}
		//peso = 1.0/(1.0+exp(- suma));
		//number += peso*suma;
		//cout<<suma<<" peso.... "<<peso<<endl;
		//cout<<suma<<" number.... "<<number<<endl;
	}
}
    ...
    peso = 1.0/(1.0+exp(- number));
    
    
    
cout<<number<<" number.... "<<peso<<endl;
...
cout<<(double)tiempo<<" ns. RN"<<endl;
cout<<(double)tiempo/1000000<<" ms. RN"<<endl;
cout<<(double)tiempo/1000000000<<" s. RN"<<endl;
